...
public class HilbertCurve implements SpaceFillingCurve {
    ...
    public Map<String, Integer> from3DCurveValue(long curveValue) {
        ...
        BitVector value = BitVectorFactories.OPTIMAL.apply(compactHilbertCurve.getSpec().sumBitsPerDimension());
        value.copyFrom(curveValue);

        compactHilbertCurve.indexInverse(value, p);

        Map<String, Integer> resultMap = new HashMap<>();
        resultMap.put("x", (int)p[0].toLong());
        resultMap.put("y", (int)p[1].toLong());
        resultMap.put("z", (int)p[2].toLong());
