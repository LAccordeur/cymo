...
public class IndexingSchemeDecider {
    ...
    public static void main(String[] args) {
        ...
        SpatialRange longitudeRange = new SpatialRange(-88, -87.5);
        SpatialRange latitudeRange = new SpatialRange(41.6, 42.1);
        TimeRange timeRange = new TimeRange(fromDateToTimestamp("2013-01-01 00:00:00"), fromDateToTimestamp("2022-01-01 23:59:59"));

        CurveMeta curveMeta = new CurveMeta(CurveType.CURVE_TIME);
        setIndexingSchemeManually(curveMeta, longitudeRange, latitudeRange, timeRange);

    }

    public static void decideNYC() {
        try {
            hBaseDriver.createTable(PartitionCurveStrategyHelper.CURVE_META_TABLE);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public static void setIndexingSchemeManually(CurveMeta curveMeta, SpatialRange longitudeRange, SpatialRange latitudeRange, TimeRange timeRange) {
        // normalize
        ...
        for (int i = startSubspaceLocation.getPartitionID(); i <= stopSubspaceLocation.getPartitionID(); i++) {
            for (int j = startSubspaceLocation.getSubspaceLongitude(); j <= stopSubspaceLocation.getSubspaceLongitude(); j++) {
                for (int k = startSubspaceLocation.getSubspaceLatitude(); k <= stopSubspaceLocation.getSubspaceLatitude(); k++) {
                    SubspaceLocation subspace = VirtualSpaceTransformationHelper.toSubspaceLocation(VirtualSpaceTransformationHelper.fromSubspaceLocation(i, j, k));
                    PartitionCurveStrategyHelper.insertCurveMetaForSubspace(curveMeta, new NormalizedLocation(subspace.getOriginalSubspaceLongitude(), subspace.getOriginalSubspaceLatitude(), subspace.getOriginalNormalizedTime()));
                }
            }
        }
    }
    ...
    public static void decideIndexingSchemeNew(SpatialRange longitudeRange, SpatialRange latitudeRange, TimeRange timeRange, double predictionSpatialWidth, int predictionTimeWidth) throws IOException {
        ...
        for (SubspaceLocation subspace : subspaceLocations) {
            ...
            // workload1 time-preferred; workload2 space-preferred
            double workload1Frequency = frequencyResult.get(0);
            double workload2Frequency = frequencyResult.get(1);

            double timeQueryOnTimeIndex = 1;
            double timeQueryOnSpaceIndex = 2;
            double spaceQueryOnSpaceIndex = 1;
            double spaceQueryOnTimeIndex = 1.5;
            double timeIndexOverhead = workload1Frequency * timeQueryOnTimeIndex + workload2Frequency * spaceQueryOnTimeIndex;
            double spaceIndexOverhead = workload1Frequency * timeQueryOnSpaceIndex + workload2Frequency * spaceQueryOnSpaceIndex;

            CurveMeta indexingScheme = null;
            if (spaceIndexOverhead > timeIndexOverhead) {
                indexingScheme = new CurveMeta(CurveType.CURVE_TIME);
            } else {
                indexingScheme = new CurveMeta(CurveType.CURVE_SPACE);
            }
